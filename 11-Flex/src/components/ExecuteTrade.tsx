...
export function ExeuteTrade() {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      console.log("执行交易策略:", selectedPeriod, "金额:", amount);
      ...
      const amount1 = allocation_num(pos1?.allocation) * parseFloat(amount);
      console.log(`Joule存款: ${pos1.asset}，数量: ${amount1}`);
      await Joule_lendToken(amount1, coin_address_map[pos1.asset], "2", false, coin_is_fungible[pos1.asset]);

      //Borrow Joule:
      const jouleAptAmount = (amount1 / aptprice) * 0.7;
      console.log(`Joule借款: APT，数量: ${jouleAptAmount}`);
      await Joule_borrowToken(jouleAptAmount, coin_address_map["APT"], "2", true);

      //Lend Aries:
      const pos2 = strategy.platforms?.Aries?.positions[0]!;
      const amount2 = allocation_num(pos2?.allocation) * parseFloat(amount);
      console.log(`Aries存款: ${pos2.asset}，数量: ${amount2}`);
      await Aries_lendToken(amount2, pos2.asset);

      //Borrow Aries:
      const ariesAptAmount = (amount2 / aptprice) * 0.7;
      console.log(`Aries借款: APT，数量: ${ariesAptAmount}`);
      await Aries_borrowToken(ariesAptAmount, "APT");
